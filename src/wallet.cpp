...
// RolleyCoin: total coins staked (non-spendable until maturity)
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
		...
		//RolleyCoin now iterates each utxo inside of CheckStakeKernelHash()
...
// RolleyCoin: check 'spent' consistency between wallet and txindex
// RolleyCoin: fix wallet spent state according to txindex
...
// RolleyCoin: disable transaction (only for coinstake)

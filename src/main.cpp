...
// Copyright (c) 2015 The RolleyCoin developers
...
const string strMessageMagic = "RolleyCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // RolleyCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // RolleyCoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// RolleyCoin: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // RolleyCoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // RolleyCoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // RolleyCoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // RolleyCoin: track money supply and mint amount info
...
// RolleyCoin: total coin age spent in transaction, in the unit of coin-days.
...
// RolleyCoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // RolleyCoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // RolleyCoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // RolleyCoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // RolleyCoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // RolleyCoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // RolleyCoin: check proof-of-stake
    ...
    // RolleyCoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // RolleyCoin: getblocks may not obtain the ancestor block rejected
    ...
    // RolleyCoin: if responsible for sync-checkpoint send it
...
// RolleyCoin: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "RolleyCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Tuesday, 22-Aug-17 17:18:00 UTC";
        CTransaction txNew;
        txNew.nTime = 1503422280;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1503422280; //Friday, 24-Mar-17 02:44:04 UTC
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 11965227;
        if(fTestNet)
        {
            block.nNonce   = 11965227;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xdea9a92bf1f77f9897f97c2567ada74e752d31d47344e62d55b6c17a2af18b9d"));
        block.print();
        ...
        // RolleyCoin: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // RolleyCoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // RolleyCoin: tell downloading node about the latest block if it's
